...
public interface CustomerDetailsRepository extends CrudRepository<CustomerDetail, Long> {
    ...
    @Query(value = "Select * from Customer_Details where customer_id=?1", nativeQuery = true)
    CustomerDetail getCustomerDetails(Long customerId);

    @Transactional
    @Modifying
    @Query(value = "UPDATE customer_details SET billing_address=?1, phone_number=?2, shipping_address=?3 WHERE customer_id=?4",nativeQuery = true)
    Integer updateCustomerDetails(String billingAddress, Long phoneNumber, String shippingAddress, Long cId);
}
